...
BattleshipGame? _setupGame() {
  ...
  if (size == null || size < 5 || size > 10) {
    ...
    game.initialize(mode == 2, modeName);
    return game;
  }
  ...
  if (!game.placeShipsRandomly(game.player1Board!, 1)) {
    print('❌ Ошибка размещения кораблей игрока 1!');
    return null;
  }

  print('⏳ Размещаем корабли игрока 2...');
  if (!game.placeShipsRandomly(game.player2Board!, 2)) {
...
Future<void> _playGame(BattleshipGame game) async {
  ...
  while (true) {
    ...
    if (currentPlayer == 1 || !game.isPlayerVsAI) {
      ...
      // Ход ИИ
      final aiMoveResult = game.aiMove();
      if (aiMoveResult != null) {
        ...
        if (result == ShotResult.miss) {
          ...
          // Если invalid или alreadyShot, переходим к игроку
          currentPlayer = 1;
          print('👤 Ваш ход!\n');
        }
      }
...
void _displayStatistics(BattleshipGame game, int player) {
  ...
  if (player == 1 || !game.isPlayerVsAI) {
    print('  👤 Игрок 1:');
    print('    🎯 Разрушено кораблей противника: ${stats.player1ShipsDestroyed}');
    print('    💔 Потеряно кораблей: ${stats.player1ShipsLost}');
    print('    🚢 Осталось кораблей на поле: ${stats.getPlayer1RemainingShips()}/${stats.totalShipsPlayer1}');
  ...
  if (!game.isPlayerVsAI || player == 2) {
    ...
    print('    🎯 Разрушено кораблей противника: ${stats.player2ShipsDestroyed}');
    print('    💔 Потеряно кораблей: ${stats.player2ShipsLost}');
    print('    🚢 Осталось кораблей на поле: ${stats.getPlayer2RemainingShips()}/${stats.totalShipsPlayer2}');
...
Future<void> _saveGameStatistics(BattleshipGame game) async {
  // Создаем путь к каталогу для статистики
